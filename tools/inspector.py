...
def checkType(obj, allowedTypes, message=None):
    if options.checker == "none":
        return True
    if options.checker == "fast" and isinstance(obj, (list, tuple, set)) and len(obj) > 100:
        obj = obj[:1]
    allowedTypes = (allowedTypes,) if not isinstance(allowedTypes, tuple) else allowedTypes
    ...
    s = "\n\tAt line " + str(stack[1].lineno) + " of function " + stack[1].function + " in file " \
        + stack[1].filename[stack[1].filename.rindex("/") + 1:]
    ...
    if modeling_file is not None and len(problem_lines) > 0:
        if functions.no_parameter_satisfy is not None:
            s += "\n\n\tThe problem comes of the parameter " + str(
                functions.no_parameter_satisfy) + " of satisfy() in file " \
                 + modeling_file.filename[modeling_file.filename.rindex("/") + 1:] + ","
        else:
            s += "\n\n\tThe problem comes at the line " + str(
                noline) + " of in file " \
                 + modeling_file.filename[modeling_file.filename.rindex("/") + 1:] + ","
        s += "\n\tWhen executing:\n\t\t" + "\n\t\t".join(problem_lines)
