...
def _compact_values(values, limit):
    if options.dont_compact_values:
        return values
...
def __compact_argument_value(arg):
    if is_1d_list(arg.content, int) and isinstance(arg.name, TypeCtrArg) and arg.name.with_compactable_values():  # TODO still other arguments to be added?
