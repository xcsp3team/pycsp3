...
@pytest.mark.parametrize("declaration, values", [
    ...
    pytest.param("x = Var(dom=True)", [1], marks=bug("#67: the domain True gives <var id='x'> True </var>, which is not valid XCSP3")),
])
...
@pytest.mark.parametrize("declaration", [
    ...
    not_explicit("x = Var(1, 2, dom=range(3))"),
    pytest.param("x = Var(0, 1, dom=range(3))", marks=bug("#68: the domain is given twice, but dom is silently ignored because the first term is 0")),
...
@pytest.mark.parametrize("declaration", [
    ...
    pytest.param("x = Var(dom=range(3), id='1x')", marks=bug("#77: an identifier starting with a digit is accepted, while it is not valid in XCSP3")),
    "Var(dom=range(3))",
...
@pytest.mark.parametrize("declaration", [
    ...
    pytest.param("x, y = VarArray(size=3, dom=range(2))", marks=bug("#75: the size 3 does not match the 2 names, silently")),
])
...
@pytest.mark.parametrize("call", [
    ...
    "x[0].name('x[1]')",
    "x[0].name('bad name')",
    pytest.param("x[0].name('')", marks=bug("#77: the empty string is accepted as an identifier")),
    "x[0].name(5)",
    "x[0].name(None)",
    "x[0].name('a')\nx[1].name('a')",
])
