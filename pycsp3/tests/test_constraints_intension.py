...
@pytest.mark.parametrize("constraint, predicate", [
    ...
    ("iff(x > 0, y > 0, b, x == y)", lambda x, y, b: (x > 0) == (y > 0) == (b == 1) == (x == y)),
    ("iff(v >= 0 for v in [x, y, b - 1])", lambda x, y, b: (x >= 0) == (y >= 0) == (b >= 1)),
    ("iff([x >= 0, y >= 0])", lambda x, y, b: (x >= 0) == (y >= 0)),
...
@pytest.mark.parametrize("constraint, text", [
    ...
    ("iff(x > 0, y > 0, x == y)", "and(iff(gt(x,0),gt(y,0)),iff(gt(y,0),eq(x,y)))"),  # all the arguments are equivalent
